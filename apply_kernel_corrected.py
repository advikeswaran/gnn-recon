"""
apply_kernel_corrected.py -- Reconstruction with:
1. T variance inflation (scale factor from overlap period)
2. P mean offset correction per accum grid node
"""
...
OUT_DIR    = RECON_DIR / 'cache' / 'recon_corrected'
...
# load corrections
t_scale    = float(np.load(str(RECON_DIR/'cache'/'t_variance_scale.npy')))
p_offsets  = np.load(str(RECON_DIR/'cache'/'p_offset_per_node.npy'))  # (n_accum,) m/yr
print(f'T variance inflation: {t_scale:.3f}x')
print(f'P offsets: mean={p_offsets.mean()*1000:.1f}mm/yr')
...
print(f"Model loaded. n_iso={n_iso} n_accum={n_accum}")

recon_years = list(range(1801, 2001))
print(f"Reconstructing {len(recon_years)} years with corrections...")
...
for yi, yr in enumerate(recon_years):
    ...
    for k, g in enumerate(grid_indices):
        ...
        if t_avail and g in iso_grid:
            ki = iso_grid.index(g)
            t_obs_norm[ki] = feats[k, 0] * loader.iso_anom_std / t_std
        if p_avail and g in accum_grid:
            ki = accum_grid.index(g)
            # apply mean offset correction: add (ERA5_mean - proxy_mean) in normalised units
            proxy_anom_myr = feats[k, 2] * loader.accum_anom_std  # m/yr anomaly
            corrected_anom = proxy_anom_myr + p_offsets[ki]       # add offset
            p_obs_norm[ki] = corrected_anom / p_std
    ...
    # apply T variance inflation
    pred_T_norm = pred_T_norm * t_scale
