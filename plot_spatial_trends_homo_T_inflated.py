"""
plot_spatial_trends_homo_T_inflated.py -- Spatial trend maps for homoscedastic model reconstruction.
Reads from cache/recon_homo_T_inflated/. Outputs to cache/plots/homo_T_inflated/
"""
...
PLOT_DIR  = RECON_DIR / 'cache' / 'plots' / 'homo'
...
for yi, yr in enumerate(recon_years):
    r = np.load(str(RECON_DIR/'cache'/'recon_homo_T_inflated'/f'recon_{yr}.npy'))
...
plt.tight_layout()
plt.savefig(str(PLOT_DIR/'spatial_trends_homo_T_inflated.png'), dpi=150, bbox_inches='tight')
print('Saved spatial_trends_homo_T_inflated.png')
