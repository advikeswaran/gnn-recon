...
from recon_gnn_kernel import (build_icosahedral_mesh, haversine_deg,
                              make_forward, norm_pos, mesh_embeddings, mesh_knn,
...
OUT_DIR    = RECON_DIR / 'cache' / 'recon_emb_interp'
...
# k-NN IDW interpolation arrays (must match training)
obs_knn_iso   = mesh_knn(obs_lats[:n_iso], obs_lons[:n_iso], mesh_lats, mesh_lons, INTERP_K)
obs_knn_accum = mesh_knn(obs_lats[n_iso:], obs_lons[n_iso:], mesh_lats, mesh_lons, INTERP_K)
grid_knn      = mesh_knn(TARGET_LATS, TARGET_LONS, mesh_lats, mesh_lons, INTERP_K)
ms = jnp.array(ms_np); mr = jnp.array(mr_np)

# load model (kernel_dim=32, mesh_rounds=4, homoscedastic, +embeddings, +interp)
...
raw_T = np.load(str(RECON_DIR/'weights_kernel'/'best_T_emb_homo_interp6.npz'))
raw_P = np.load(str(RECON_DIR/'weights_kernel'/'best_P_emb_homo_interp6.npz'))
