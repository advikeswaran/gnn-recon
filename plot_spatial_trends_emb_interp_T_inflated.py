"""
plot_spatial_trends_emb_interp_T_inflated.py -- Spatial trend maps for homoscedastic + embeddings reconstruction.
Reads from cache/recon_emb_interp_T_inflated/. Outputs to cache/plots/emb/
"""
...
PLOT_DIR  = RECON_DIR / 'cache' / 'plots' / 'emb_interp_T_inflated'
...
for yi, yr in enumerate(recon_years):
    r = np.load(str(RECON_DIR/'cache'/'recon_emb_interp_T_inflated'/f'recon_{yr}.npy'))
...
fig.suptitle('Antarctic Climate Reconstruction — Spatial Trend Maps\n(Kernel GNN — Homoscedastic + GraphCast Embeddings, interp + T-inflated)',
             fontsize=13, fontweight='bold')

# per-period SMB colour-scale limit (mm/yr/dec)
SMB_LIM = {1801: 20.0, 1901: 40.0, 1957: 40.0, 1979: 100.0}
...
plt.tight_layout()
plt.savefig(str(PLOT_DIR/'spatial_trends_emb_interp_T_inflated.png'), dpi=150, bbox_inches='tight')
print('Saved spatial_trends_emb_interp_T_inflated.png')
