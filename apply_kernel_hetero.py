"""
apply_kernel_hetero.py -- Apply heteroscedastic kernel GNN to ice core proxies,
reconstructing Antarctic T and P fields for 1801-2000.

Uses best_T_heteroscedastic.npz / best_P_heteroscedastic.npz with the matching
heteroscedastic forward pass (per-node R2-derived noise diagonal).
"""
...
OUT_DIR    = RECON_DIR / 'cache' / 'recon_hetero'
...
print(f"Per-node noise: iso mean={iso_node_noise.mean():.3f}  accum mean={accum_node_noise.mean():.3f}")

# heteroscedastic forward functions
...
raw_T = np.load(str(RECON_DIR / 'weights_kernel' / 'best_T_heteroscedastic.npz'))
raw_P = np.load(str(RECON_DIR / 'weights_kernel' / 'best_P_heteroscedastic.npz'))
...
for yi, yr in enumerate(recon_years):
    ...
    for k, g in enumerate(grid_indices):
        ...
        if p_avail and g in accum_grid:
            ...
            p_obs_norm[ki] = feats[k, 2] * loader.accum_anom_std / p_std
