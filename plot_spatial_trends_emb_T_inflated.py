"""
plot_spatial_trends_emb_T_inflated.py -- Spatial trend maps for homoscedastic + embeddings reconstruction.
Reads from cache/recon_emb_T_inflated/. Outputs to cache/plots/emb/
"""
...
PLOT_DIR  = RECON_DIR / 'cache' / 'plots' / 'emb_T_inflated'
...
for yi, yr in enumerate(recon_years):
    r = np.load(str(RECON_DIR/'cache'/'recon_emb_T_inflated'/f'recon_{yr}.npy'))
...
plt.tight_layout()
plt.savefig(str(PLOT_DIR/'spatial_trends_emb_T_inflated.png'), dpi=150, bbox_inches='tight')
print('Saved spatial_trends_emb_T_inflated.png')
